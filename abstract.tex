...
\noindent \textbf{Methods} We represent forest dynamics with a size-structured population model and neighbourhood competition with the \textit{Perfect-Plasticity-Approximation}. We then derive the lifetime reproduction per individual $ R_0 $ in the absence of density-dependence. Using forest inventory data, we assess how tree demography for each species varies with climate. We test the model by comparing $ R_0 $ and the probability of occurrence within species ranges. \\ % 61 words
